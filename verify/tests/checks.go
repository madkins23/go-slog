...
func (suite *SlogTestSuite) checkFieldCount(fieldCount uint, logMap map[string]any) {
	if suite.HasWarning(infra.WarnDuplicates) {
		counter := suite.fieldCounter()
		suite.Require().NoError(counter.Parse())
		if len(counter.Duplicates()) > 0 {
			suite.AddWarning(infra.WarnDuplicates, fmt.Sprintf("%v", counter.Duplicates()), "")
			return
		}
	}
	...
	if suite.HasWarning(infra.WarnLevelCase) {
		if logLevel, ok := logMap[slog.LevelKey].(string); ok {
			if suite.Assert().Equal(level, strings.ToUpper(logLevel)) && level != logLevel {
				suite.AddWarning(infra.WarnLevelCase, "'"+logLevel+"'", "")
				return
			}
		}
		suite.AddWarning(infra.WarnUnused, infra.WarnLevelCase, "")
	}
...
func (suite *SlogTestSuite) checkMessageKey(message string, logMap map[string]any) {
	if suite.HasWarning(infra.WarnMessageKey) {
		if _, found := logMap[slog.MessageKey]; found {
			...
			if message == msg {
				suite.AddWarning(infra.WarnMessageKey, "`message`", "")
				return
			}
		}
		suite.AddWarning(infra.WarnUnused, infra.WarnMessageKey, "")
	}
	...
	if suite.HasWarning(infra.WarnEmptyAttributes) {
		...
		if counter.NumFields() == fieldCount+1 {
			if _, found := logMap[""]; found {
				suite.AddWarning(infra.WarnEmptyAttributes, "", suite.Buffer.String())
				return
			}
		}
		suite.AddWarning(infra.WarnUnused, infra.WarnEmptyAttributes, "")
	}
...
func (suite *SlogTestSuite) checkResolution(value any, actual any) {
	if suite.HasWarning(infra.WarnResolver) {
		if value != actual {
			suite.AddWarning(infra.WarnResolver, "", suite.Buffer.String())
			return
		}
		suite.AddWarning(infra.WarnUnused, infra.WarnResolver, "")
	}
...
func (suite *SlogTestSuite) checkSourceKey(fieldCount uint, logMap map[string]any) {
	if suite.HasWarning(infra.WarnSourceKey) {
		sourceData := logMap[slog.SourceKey]
		if sourceData == nil {
			suite.AddWarning(infra.WarnSourceKey, "no 'source' key", suite.Buffer.String())
			return
		}
		source, ok := sourceData.(map[string]any)
		if !ok {
			suite.AddWarning(infra.WarnSourceKey, "'source' key not a group", suite.Buffer.String())
			return
		}
		...
		if text.Len() > 0 {
			suite.AddWarning(infra.WarnSourceKey, text.String(), suite.Buffer.String())
		}
		suite.AddWarning(infra.WarnUnused, infra.WarnSourceKey, "")
	}
